...
def simulate_cascade_failure(removal_ratio, G, attack_type='random'):
    ...
    attack_type (str): 攻击类型，'random'、'degree' 或 'betweenness'。
    ...
    if attack_type == 'random':
        ...
        nodes_sorted_by_betweenness = sorted(betweenness.keys(), key=lambda n: betweenness[n], reverse=True)
        nodes_to_remove = nodes_sorted_by_betweenness[:int(removal_ratio * len(G_copy))]  # 选择介数最高的节点进行移除
    else:
        raise ValueError("Unsupported attack type. Choose 'random', 'degree', or 'betweenness'.")
...
def generate_random_graph(n, m):
    ...
    返回:
    networkx.Graph: 生成的随机图。
    """
    return nx.gnm_random_graph(n, m)


def attack_simulation(G, attack_types, steps=100):
    """
    模拟不同攻击策略下网络的瓦解过程。

    参数:
    G (networkx.Graph): 要攻击的图。
    attack_types (list of str): 攻击类型列表，如 ['random', 'degree', 'betweenness']。
    steps (int): 攻击步骤数。

    返回:
    dict: 每种攻击类型对应的最大连通子图比例列表。
    """
...
def plot_results(removal_ratios, results):
    """
    绘制不同攻击策略下的网络瓦解效果。
    ...
    plt.title('Network Breakdown under Different Attack Strategies')
    plt.legend()
    plt.grid(True)
    plt.show()
...
def main():
    ...
    steps = 50  # 攻击步骤数，减小以加快计算速度

    print("生成Erdős–Rényi随机网络...")
    ...
    print("绘制结果...")
    plot_results(removal_ratios, results)
